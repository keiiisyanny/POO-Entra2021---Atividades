...
//		vacinaRep.excluir(5);
		...
		novaVacina.setPaisOrigem("Russia");
		novaVacina.setEstagioPesquisa(9);
		novaVacina.setDataInicioPesquisa(new Date());

		novaVacina = vacinaRep.inserir(novaVacina);
		if (novaVacina.getId() > 0) {
			System.out.println("Vacina Salva");
		}
		
		Vacina vacina = vacinaRep.pesquisarPorId(6);
		
		System.out.println(vacina.getPaisOrigem());
